<?php
/* --------------------------------------------------------------
   $Id: categories_specials.php 11649 2019-03-28 14:36:34Z GTB $
   ...
   Anpssungen David Jacob, da zu viele Warnings wegen Zugriffen auf uninitialisierte Variablen
*/
...
if (isset($sInfo) && is_object($sInfo))
{
	...
	if (PRICE_IS_BRUTTO=='true') 
	{
  	...
  	if ($price > 0) { $new_price_netto = TEXT_NETTO.'<strong>'.xtc_round($new_price,PRICE_PRECISION).'</strong>'; }
...
// build the start and expires date in the format YYYY-MM-DD
$expires_date = $start_date = '';
if(isset($_GET['pID']) && xtc_db_num_rows($specials_query, true) > 0) 
{
	if (isset($sInfo->start_date) && $sInfo->start_date != 0) { $start_date = date('Y-m-d', strtotime($sInfo->start_date)); }
  if (isset($sInfo->expires_date) && $sInfo->expires_date != 0) { $expires_date = date('Y-m-d', strtotime($sInfo->expires_date)); }  
}

// tell the storing script if to update existing special or to insert a new one
if(isset($_GET['pID']) && xtc_db_num_rows($specials_query, true) > 0)
{
	echo xtc_draw_hidden_field('specials_action', 'update');
	echo xtc_draw_hidden_field('specials_id', $sInfo->specials_id);
}
else { echo xtc_draw_hidden_field('specials_action', 'insert'); } 
...
<div id="special" class="longDescription">
  <table class="tableInput">    
    ...
    <tr>
      ...
      <td class="main"><?php echo xtc_draw_input_field('specials_quantity', (isset($sInfo->specials_quantity) ? $sInfo->specials_quantity : ''), 'style="width: 135px"') . draw_tooltip(TEXT_CATSPECIALS_SPECIAL_QUANTITY_TT);?></td>
    </tr>
    <?php if(isset($_GET['pID']) && xtc_db_num_rows($specials_query, true) > 0) { ?>
      <tr>
